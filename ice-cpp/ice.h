...
class ResponseStream {
    ...
    public:
        ...
        bool write_str(const char *s) const {
            return write_bytes((const u8 *) s, strlen(s));
        }
...
class Response {
    ...
    public:
        ...
        inline Response& set_body_s(const char *body) {
...
class Server {
    ...
    public:
        ...
        void _add_endpoint(const char *path, DispatchTarget handler) {
        ...
        void _route_async(const char *path, DispatchTarget handler) {
            std::vector<std::string> flags;
